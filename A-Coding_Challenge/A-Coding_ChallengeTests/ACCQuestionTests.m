...
@interface ACCQuestionTests : XCTestCase

@end

@implementation ACCQuestionTests
{
...
- (void)setUp {
    ...
    ],\"has_more\":true,\"quota_max\":300,\"quota_remaining\":143}";
    
    NSError * parserError = nil;
    NSDictionary * d = [NSJSONSerialization JSONObjectWithData:[_sampleJSON dataUsingEncoding:NSUTF8StringEncoding] options:0 error:&parserError];
    ...
    NSArray * itemArray = [d objectForKey:@"items"];
    
    _parsedJSON = [itemArray firstObject];
...
-(void)testQuestionTitleField {
    ACCQuestion * tq = [[ACCQuestion alloc]initWithJSONData:_parsedJSON];
    
    XCTAssertEqualObjects(@"Test title Test Title", tq.title);
}

-(void)testQuestionIdField {
    ACCQuestion * tq = [[ACCQuestion alloc]initWithJSONData:_parsedJSON];
    
    XCTAssertEqual(33925215, tq.questionId);
}

-(void)testQuestionCreationDateField {
    ACCQuestion * tq = [[ACCQuestion alloc]initWithJSONData:_parsedJSON];
    
    XCTAssertEqual((NSTimeInterval)1448480840, tq.creationDate.timeIntervalSince1970);
}

-(void)testQuestionUserFieldNotNil {
    ACCQuestion * tq = [[ACCQuestion alloc]initWithJSONData:_parsedJSON];
    
    XCTAssertNotNil(tq.owner);
}
